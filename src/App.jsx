...
function App() {
  ...
  useEffect(() => {
    let timer;
    if (activeCell !== null && timeLeft > 0 && !explodedMine) {
      timer = setInterval(() => {
        setTimeLeft(prev => {
  ...
  }, [activeCell, timeLeft, explodedMine]);
  ...
  const resetFullGame = () => {
    ...
    setTeam1Lifelines({ reveal: true, silence: true, changeQ: true }); setTeam2Lifelines({ reveal: true, silence: true, changeQ: true });
  ...
  const globalStyles = `
    ...
    .app-container { 
        flex: 1; background: radial-gradient(ellipse at top, #11111a 0%, var(--bg-deep) 80%);
        display: flex; flex-direction: column; position: relative; z-index: 1;
    }

    .app-container::before, .app-container::after {
        content: ''; position: fixed; width: 60vw; height: 60vw; border-radius: 50%; filter: blur(140px); z-index: -1; opacity: 0.12; animation: drift 30s infinite alternate ease-in-out;
    }
    ...
    .pro-input {
        ...
        color: white; font-family: inherit; font-size: 1.1rem; font-weight: 700; outline: none; transition: all 0.3s ease; box-sizing: border-box;
    }
    ...
    .pulse-btn { 
        background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.08); color: var(--text-secondary); padding: 14px 28px; border-radius: 12px; 
    ...
    .launch-btn {
        width: 100%; padding: 25px; background: linear-gradient(90deg, #ffffff, #e0e0e0); color: #000; border: none; border-radius: 16px; font-size: 1.6rem; font-weight: 900; 
        cursor: pointer; font-family: inherit; transition: all 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275); box-shadow: 0 15px 40px rgba(255,255,255,0.2); text-transform: uppercase; letter-spacing: 1px;
    }
    ...
    @media (max-width: 1024px) { .app-container { padding: 15px !important; } h1 { font-size: 3rem !important; } .esport-panel { padding: 20px !important; } }
    @media (max-width: 768px) {
        h1 { font-size: 2rem !important; }
        div[style*="justify-content: space-between"] { flex-direction: column !important; align-items: center !important; gap: 15px !important; }
        .esport-panel { width: 100% !important; min-width: unset !important; padding: 15px !important; }
        div[style*="font-size: 4.5rem"] { font-size: 2.5rem !important; }
        div[style*="font-size: 1.6rem"] { font-size: 1.2rem !important; }
        .hex-container { padding: 20px 10px !important; transform: scale(0.95); }
